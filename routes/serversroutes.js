...
route.get("/:id", async (req, res, next) => {
  ...
  if (server.state === "deleted")
    return renderTemplate(res, req, "servers/404");

  const guild = req.app.get("client").guilds.cache.get(req.params.id);

  var desc = `La description de ce serveur a été corrompue, probablement à cause d'une image.`;
  ...
  if (user != null) {
    isManaged =
      guild && !!guild.member(user.id)
        ? guild.member(user.id).permissions.has("MANAGE_GUILD")
        : false;
...
route.get("/:id/edit", async (req, res, next) => {
  ...
  const isManaged =
    guild && !!guild.member(user.id)
      ? guild.member(user.id).permissions.has("MANAGE_GUILD")
      : false;
  if (!isManaged && !process.env.ADMIN_USERS.includes(user.id))
    return renderTemplate(res, req, "errors/403");
  ...
  renderTemplate(res, req, "servers/edit", {
    server: server,
    guild: guild,
    categories,
  });
...
route.post("/:id/edit", async (req, res, next) => {
  ...
  if (!refresh_token)
    return res
      ...
      .json({
        code: "NO_PERMISSION",
        message: "Vous n'avez pas les permissions pour i accéder.",
      });
  ...
  if (!result)
    return res
      ...
      .json({
        code: "NO_PERMISSION",
        message:
          "Vous n'avez pas les permissions pour accéder a la modification.",
      });
  [user, { refresh_token, access_token }] = result;
  ...
  if (!server)
    return res
      ...
      .json({
        code: "NOT_FOUND",
        message:
          "The server you tried to access was non-existant. Please check the url and try again.",
      });
  ...
  if (Object.keys(body).length == 0)
    return res
      ...
      .json({
        code: "EMPTY_RESPONSE",
        message:
          "No data received. The server closed the connection without receiving any data.",
      });

  if (body.vanityURL && body.vanityAction) {
    let servervenity = await Servers.findOne(
      { "vanity.code": body.vanityURL },
      { _id: false }
    );
    if (servervenity != null)
      return res
        .status(409)
        .json({
          code: "CONFLICT",
          message:
            "The selected vanity code already exists for another server. Please select another one, and try again.",
        });
    await Servers.updateOne(
      { guildid: server.guildid },
      { $set: { vanity: { code: body.vanityURL, action: body.vanityAction } } }
    );
    return res
      .status(201)
      .json({
        message: "Successfully updated your vanity settings!",
        code: "SUCCESS",
        redirect: `/server/${req.params.id}/?type=edited`,
      });
  }
    ...
    await Servers.updateOne(
      { guildid: server.guildid },
      {
        $set: {
          ...
          long: body.longdesc,
          invite: body.invitelink,
          categories: tags,
          styles: { background: body.backgroundStyle },
        },
      }
    );
    return res
      .status(201)
      .json({
        message: "Successfully updated your server settings!",
        code: "SUCCESS",
        redirect: `/server/${req.params.id}/?type=edited`,
      });
  }
  ...
  res
    .status(400)
    .json({ message: "An unexpected error occurred.", code: "UNEXPECTED_ERR" });
});
...
route.post("/:id/bump", async (req, res, next) => {
  ...
  if (timeremain.days == 0) {
    if (timeremain.hours < 2)
      return res.redirect(`/server/${server.guildid}/?type=cooldown`);
  }
  await Servers.updateOne(
    { guildid: server.guildid },
    { $set: { lastbumped: new Date(Date.parse(new Date())) } }
  );
